...
#define SPL06_TEMPERATURE_CFG \
  (TEMPERATURE_EXTERNAL_SENSOR << 7 | T_MEASURE_RATE << 4 | T_OVERSAMP_RATE)

#define SPL06_MODE (SPL06_CONTINUOUS_MODE)
#define SPL06_TIME_MS 10

// 缩放因子，用于计算气压和温度
const uint32_t scaleFactor[8] = {524288, 1572864, 3670016, 7864320,
...
// static bool spl06i2cdevRead(I2C_Dev* dev,
//                             uint8_t devAddress,
//                             uint8_t memAddress,
//                             uint16_t len,
//                             uint8_t* data);
...
void spl0601_get_calib_param(void) {
  ...
  i2cdevReadBit(I2Cx, devAddr, SPL06_COEFFICIENT_CALIB_REG,
                SPL06_CALIB_COEFFICIENT_LENGTH, buffer);

  spl06Calib.c0 = (int16_t)buffer[0] << 4 | buffer[1] >> 4;
  spl06Calib.c0 =
      (spl06Calib.c0 & 0x0800) ? (spl06Calib.c0 | 0xF000) : spl06Calib.c0;

  spl06Calib.c1 = (int16_t)(buffer[1] & 0x0F) << 8 | buffer[2];
  spl06Calib.c1 =
      (spl06Calib.c1 & 0x0800) ? (spl06Calib.c1 | 0xF000) : spl06Calib.c1;
 ...
 */
void spl0601_rateset(spl06Sensor_e sensor,
                     uint8_t measureRate,
                     uint8_t oversamplRate) {
  uint8_t reg;
  if (sensor == PRESURE_SENSOR) {
    kp = scaleFactor[oversamplRate];
    i2cdevWriteByte(I2Cx, devAddr, SPL06_PRESSURE_CFG_REG,
                    measureRate << 4 | oversamplRate);
    ...
    i2cdevWriteByte(I2Cx, devAddr, SPL06_TEMPERATURE_CFG_REG,
                    measureRate << 4 | oversamplRate | 0x80);  // 使用MEMS温度
    if (oversamplRate > SPL06_OVERSAMP_8) {
      i2cdevReadByte(I2Cx, devAddr, SPL06_INT_FIFO_CFG_REG, &reg);
...
void SPL06GetPressure(void) {
  ...
  i2cdevReadBit(I2Cx, devAddr, SPL06_PRESSURE_MSB_REG, SPL06_DATA_FRAME_SIZE,
                data);

  SPL06RawPressure =
      (int32_t)data[0] << 16 | (int32_t)data[1] << 8 | (int32_t)data[2];
  SPL06RawPressure = (SPL06RawPressure & 0x800000)
                         ? (0xFF000000 | SPL06RawPressure)
                         : SPL06RawPressure;

  SPL06RawTemperature =
      (int32_t)data[3] << 16 | (int32_t)data[4] << 8 | (int32_t)data[5];
  SPL06RawTemperature = (SPL06RawTemperature & 0x800000)
                            ? (0xFF000000 | SPL06RawTemperature)
                            : SPL06RawTemperature;
}
...
float spl0601_get_pressure(int32_t rawPressure, int32_t rawTemperature) {
  ...
  // fPCompensate = spl06Calib.c00 + fPsc * qua2 + 0.9f * fTsc * spl06Calib.c01
  // + qua3;
  return fPCompensate;
}
