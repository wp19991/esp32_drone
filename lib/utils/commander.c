...
static ctrlVal_t ctrlValLpf = {0.f, 0.f, 0.f, 0.f, 0.f, 0};/* 控制数据低通 */
...
static void ctrlDataUpdate(void) {
    ...
    if (isRCLocked == false)    /*解锁状态*/
    {
        ...
        ctrlValLpf.thrust += (uint16_t) ((float) (ctrlVal.thrust - ctrlValLpf.thrust) * lpfVal);
...
//    static float maxAccZ = 0.f;
    ...
    if (commander.ctrlMode & 0x01)/*定高模式*/
    {
        ...
        } else if (commander.keyFlight)/*一键起飞*/
        {
            ...
            float climb = (((float) ctrlValLpf.thrust - 32767.f) / 32767.f);
            if (climb > 0.f)
                climb *= MAX_CLIMB_UP;
            else
