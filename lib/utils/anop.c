...
float adcGetFlyData(uint8_t adc_val, uint8_t mid_db) {
    ...
    if (fabsf(temp) < VEL_LIMIT) temp = 0;

    return (float) (temp / MID_MAX) * 2;
}

//与匿名上位机通讯协议V7 详细 查看 http://anotc.com/
void check_data(uint8_t *sum_check, uint8_t *add_check, const uint8_t *pData, uint8_t len) {
...
//发送log信息
bool anopSendLogStr(const uint8_t *data, uint8_t len, uint8_t color) {
    uint8_t cnt = 0;
    uint8_t sum_check = 0;
    uint8_t add_check = 0;
    anop_t p;
    ...
    if (Key_status & 0x80) { //x
        ...
        if (fly_sta >= 20) {
            if (getCommanderKeyFlight() != true) {
                ...
                if (!isOffse) {
                    ...
                    wifiCtrl.trimPitch = (attitude.pitch * 0.8f);
                    wifiCtrl.trimRoll = (attitude.roll * 0.95f);
                    isOffse = 1;
                }
        ...
        switch (temp) {
            ...
            case 8:
                ...
                fTemp = adcGetFlyData(mid_data, MID_DB_ROLL) * (float) LOW_SPEED_ROLL;
                wifiCtrl.roll = limit(fTemp, (float) -LOW_SPEED_ROLL, LOW_SPEED_ROLL);

                mid_data = *(pData->data + 3); //x
                fTemp = adcGetFlyData(mid_data, MID_DB_PITCH) * (float) LOW_SPEED_PITCH;
                wifiCtrl.pitch = limit(fTemp, (float) -LOW_SPEED_PITCH, LOW_SPEED_PITCH);

                mid_data = *(pData->data + 4); //x
                fTemp = adcGetFlyData(mid_data, MID_DB_YAW) * (float) MAX_YAW;
                wifiCtrl.yaw = limit(fTemp, (float) -MAX_YAW, MAX_YAW);

                mid_data = *(pData->data + 5);
                fTemp = (adcGetFlyData(mid_data, MID_DB_THRUST) * (float) ALT_THRUST) + (float) ALT_THRUST;
...
static void anoPSendPeriod(void) {
    ...
    if (!(ms_cnt % PERIOD_SENSOR)) {
        attitude_t attitude;
        getAttitudeData(&attitude);
        sendSenseRolPIT((int16_t) (attitude.roll * 100),
                        (int16_t) (attitude.pitch * 100),
                        (int16_t) (attitude.yaw * 100), 1);
    }
    //2、发送 加速度、陀螺仪传感器数据
    ...
    if (!(ms_cnt % PERIOD_Q)) {
        ...
        sendSenseV0123((int16_t) (q0 * 10000), (int16_t) (q1 * 10000), (int16_t) (q2 * 10000), (int16_t) (q3 * 10000),
                       1);
    }
    ...
    if (!(ms_cnt % PERIOD_HEIGHT)) {
        ...
        sendControlledQuantiy(get.roll, get.pitch, (int16_t) get.thrust, get.yaw);
    }
    ...
    if (!(ms_cnt % PERIOD_MOTOR)) {
        ...
        m1 = (uint16_t) ((float) motorPWM.m1 / 65535 * 1000);
        m2 = (uint16_t) ((float) motorPWM.m2 / 65535 * 1000);
        m3 = (uint16_t) ((float) motorPWM.m3 / 65535 * 1000);
        m4 = (uint16_t) ((float) motorPWM.m4 / 65535 * 1000);
