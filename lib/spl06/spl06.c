...
void spl0601_rateset(spl06Sensor_e sensor, uint8_t measureRate, uint8_t oversamplRate) {
    ...
    if (sensor == PRESURE_SENSOR) {
        kp = (int32_t) scaleFactor[oversamplRate];
        ...
        kt = (int32_t) scaleFactor[oversamplRate];
        i2cdevWriteByte(I2Cx, devAddr, SPL06_TEMPERATURE_CFG_REG,
                        measureRate << 4 | oversamplRate | 0x80);//Using mems temperature
        if (oversamplRate > SPL06_OVERSAMP_8) {
            i2cdevReadByte(I2Cx, devAddr, SPL06_INT_FIFO_CFG_REG, &reg);
...
float spl0601_get_temperature(int32_t rawTemperature) {
    ...
    fTsc = (float) rawTemperature / (float) kt;
    fTCompensate = (float) spl06Calib.c0 * 0.5f + (float) spl06Calib.c1 * fTsc;
    return fTCompensate;
}
...
float spl0601_get_pressure(int32_t rawPressure, int32_t rawTemperature) {
    ...
    fTsc = (float) rawTemperature / (float) kt;
    fPsc = (float) rawPressure / (float) kp;
    qua2 = (float) spl06Calib.c10 + fPsc * ((float) spl06Calib.c20 + fPsc * (float) spl06Calib.c30);
    qua3 = fTsc * fPsc * ((float) spl06Calib.c11 + fPsc * (float) spl06Calib.c21);
    //qua3 = 0.9f *fTsc * fPsc * (spl06Calib.c11 + fPsc * spl06Calib.c21);

    fPCompensate = (float) spl06Calib.c00 + fPsc * qua2 + fTsc * (float) spl06Calib.c01 + qua3;
