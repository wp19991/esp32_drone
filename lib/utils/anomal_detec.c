...
static bool detecFreeFall(float accZ, float accMAG)    /*自由落体检测*/
{
    ...
    /*自由落体*/
    if (fabsf(accMAG) < DETEC_FF_THRESHOLD && fabsf(accZ + 1.f) < DETEC_FF_THRESHOLD) {
...
static bool detecTumbled(const state_t *state)    /*碰撞检测*/
{
    ...
    float fAbsRoll = fabsf(state->attitude.roll);
    float fAbsPitch = fabsf(state->attitude.pitch);
