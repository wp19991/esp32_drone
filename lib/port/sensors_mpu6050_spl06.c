...
static void sensorsInterruptInit(void) {
    ...
    ESP_LOGE(TAG, "in sensorsInterruptInit sensorsDataReady");
...
static void sensorsCalculateVarianceAndMean(BiasObj *bias, Axis3f *varOut, Axis3f *meanOut) {
    ...
    varOut->x = ((float) sumSq[0] - (float) ((int64_t) sum[0] * sum[0]) / (float) SENSORS_NBR_OF_BIAS_SAMPLES);
    varOut->y = ((float) sumSq[1] - (float) ((int64_t) sum[1] * sum[1]) / (float) SENSORS_NBR_OF_BIAS_SAMPLES);
    varOut->z = ((float) sumSq[2] - (float) ((int64_t) sum[2] * sum[2]) / (float) SENSORS_NBR_OF_BIAS_SAMPLES);
...
static bool sensorsFindBiasValue(BiasObj *bias) {
    ...
    ESP_LOGE(TAG, "in sensorsFindBiasValue ,bias->isBufferFilled: %d", bias->isBufferFilled);
...
static bool processGyroBias(int16_t gx, int16_t gy, int16_t gz, Axis3f *gyroBiasOut) {
    ...
    ESP_LOGE(TAG, "in processGyroBias ,gyroBiasRunning.isBiasValueFound: %d", gyroBiasRunning.isBiasValueFound);
...
static bool processAccScale(int16_t ax, int16_t ay, int16_t az) {
    ...
    if (!accBiasFound) {
        accScaleSum += sqrtf(powf((float) ax * SENSORS_G_PER_LSB_CFG, 2) + powf((float) ay * SENSORS_G_PER_LSB_CFG, 2) +
                             powf((float) az * SENSORS_G_PER_LSB_CFG, 2));
        accScaleSumCount++;
...
void processAccGyroMeasurements(const uint8_t *buffer) {
    ...
    ESP_LOGE(TAG, "in processAccGyroMeasurements");
    ...
    sensorData.gyro.y = ((float) gyroRaw.y - gyroBias.y) * SENSORS_DEG_PER_LSB_CFG;/*单位 °/s */
...
#else
    accScaled.x = (float) -(accelRaw.x) * SENSORS_G_PER_LSB_CFG / accScale;
#endif

    accScaled.y = (float) (accelRaw.y) * SENSORS_G_PER_LSB_CFG / accScale;
    accScaled.z = (float) (accelRaw.z) * SENSORS_G_PER_LSB_CFG / accScale;
...
static void sensorsTask(void *param) {
    ...
    ESP_LOGI(TAG, "in sensorsTask xTaskCreate sensorsTask SetupSlave done\n");

    while (1) {
        ESP_LOGI(TAG, "in sensorsTask while\n");

        // 获取互斥锁
        //信号量是在“空”状态下创建的，这意味着信号量必须首先使用 xSemaphoreGive() API函数给出，然后才能使用xSemaphoreTake（）函数获取（获取）。
        /* mpu6050 interrupt trigger: data is ready to be read */
        if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY)) {
            ESP_LOGE(TAG, "in sensorsTask while if");
            ...
            ESP_LOGE(TAG, "in sensorsTask ,buffer[0]:%d [1]:%d", buffer[0],buffer[1]);
            ...
            if (isBarometerPresent) {
                processBarometerMeasurements(
                        &(buffer[isMagnetometerPresent ? SENSORS_MPU6050_BUFF_LEN + SENSORS_MAG_BUFF_LEN
                                                       : SENSORS_MPU6050_BUFF_LEN]));
            }
...
static void sensorsTaskInit(void) {
    ...
    xTaskCreate(sensorsTask, SENSORS_TASK_NAME, SENSORS_TASK_STACKSIZE, NULL, SENSORS_TASK_PRI,
                &sensors_handle);            /*创建传感器处理任务*/
    // STATIC_MEM_TASK_CREATE(sensorsTask, sensorsTask, SENSORS_TASK_NAME, NULL, SENSORS_TASK_PRI);
    ESP_LOGI(TAG, "xTaskCreate sensorsTask \r\n");

    eTaskState taskState = eTaskGetState(sensors_handle);
    if (taskState == eSuspended) {
        ESP_LOGI(TAG, "in sensorsTaskInit sensorsTask is suspended.");
    } else {
        ESP_LOGI(TAG, "in sensorsTaskInit sensorsTask is in state: %d", taskState);
    }
}
/*传感器数据校准*/
...
bool sensorsMpu6050Test(void) {
    ...
    if (testStatus) {
        ...
        if (gyroBiasFound) {
            acc.x = (float) (a.x) * SENSORS_G_PER_LSB_CFG;
            acc.y = (float) (a.y) * SENSORS_G_PER_LSB_CFG;
            acc.z = (float) (a.z) * SENSORS_G_PER_LSB_CFG;
