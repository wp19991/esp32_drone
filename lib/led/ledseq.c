...
static ledseqStep_t const* sequences[] = {
    seq_lowbat,     seq_charged, seq_charging,
...
static TimerHandle_t timer[LED_NUM];  // 软件的定时器
static SemaphoreHandle_t ledseqSem;   /*信号量*/
...
void ledseqInit() {
  ...
  for (int i = 0; i < LED_NUM; i++) {  // 创建软件定时器
    // timer[i] = xTimerCreateStatic("ledseqTimer",1000, pdFALSE, (void*)i,
    // runLedseq, &timerBuffer[i]);
    timer[i] =
        xTimerCreate("ledseqTimer", M2T(1000), pdFALSE, (void*)i, runLedseq);
  }
